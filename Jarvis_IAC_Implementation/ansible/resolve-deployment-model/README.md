...
- Derive a run-level claim/onboarding readiness decision
- Run endpoint-side preparation and PVA claim for supported targets when explicitly requested
...
- Rack-server manufacturing/default credentials should be handled by the separate `reset-rack-password` grain
...
- Produces a run-level onboarding readiness result from baseline, placement, and target-selection state
...
- Blade targets currently remain in the guarded non-direct path
...
Current PVA claim shape:

- each declared `fi_pair` domain becomes one Fabric Interconnect claim candidate
- standalone rack servers remain one claim candidate per server
- blades are still modeled in the discovery and readiness flow, but not directly claimed in the current appliance path
- appliance claim follow-up is batched:
  submit all eligible targets first, wait once, then enrich results from workflow and `DeviceClaims`
